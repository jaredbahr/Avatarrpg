...
test('a new campaign rescues the quarry workers and walks home after reloading', async ({
  page,
}) => {
  test.setTimeout(600_000);
  await resetStorage(page, '?renderer=canvas');
  await startGame(page, ['Kaya', 'Nilak', 'Bo'], ['kaya', 'nilak', 'bo'], 'quarry-return-campaign');
  ...
  await continueStory(page);
  expect(await page.evaluate(() => window.fnt?.app.state?.flags.ruon_traded)).toBe(true);
  ...
  expect(await page.evaluate(() => window.fnt?.app.state?.story.nodeId)).toBe('gao_home_cold');
  await expect(page.locator('.dialogue-line')).toContainText('workers are home');
  await page.locator('.dialogue-panel button.btn-primary').click();
  await expect(page.locator('.dialogue-line')).toContainText('Ruon');
  await continueStory(page);
  ...
  await takeRoute(page, 'West → Forest Road', 'forest_road');
  await takeRoute(page, 'West → Ba Dan Village', 'ba_dan_village');
  expect(await page.evaluate(() => window.fnt?.app.state?.world.cleared)).toContain('enc_grumbler');
});
