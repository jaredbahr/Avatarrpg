...
for (const renderer of ['canvas', 'webgl']) {
  test(`shopfront Talk chooses Gao and the ground ring cannot cover Mira on ${renderer}`, async ({
    page,
  }) => {
    ...
    const probe = await page.evaluate(() => {
      ...
      const win = window as Window & { hideGroundRing?: boolean };
      scene.renderer.draw = (view) =>
        draw({
          ...view,
          time: 1000,
          activeUnitId: win.hideGroundRing ? null : view.activeUnitId,
          selectedUnitId: null,
        });
