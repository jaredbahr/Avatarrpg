# Sheet: Nima — `unit.air.nima`

|               |                                                                                                                                                                                                                                    |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Palette**   | `air` — base `#e8dcc0`, light `#fdf6e3`, dark `#9a8e72`, accent `#fffdf5`, ink `#1b1410`, key `#00ff00`                                                                                                                            |
| **Reference** | `art/raw/reference/unit.air.nima.png` — the full-body figure; the image reference for every frame                                                                                                                                  |
| **Deliver**   | `art/raw/unit.air.nima/<clip>/<index>.png`, one 512×768 PNG per row of the pose table                                                                                                                                              |
| **Ships as**  | `public/art/units/nima.png` and `nima.json`, 128×192 frames written by `art:pack`                                                                                                                                                  |
| **Manifest**  | `'unit.air.nima': { kind: 'sheet', atlas: 'art/units/nima.json', pixelsPerTile: 128, footprint: { w: 1, h: 1 }, anchor: { x: 0.5, y: 0.85 }, facing: 'mirror', palette: 'air', clips: { … } }` — `art:pack` prints the whole block |
...
| `cast/0.png`  | Cast wind-up  | Weight drawn back onto the rear foot, the torso coiled, a single curl of wind drawn as one flat pale spiral at the rear hand, the cape lifting.                                   |
| `cast/1.png`  | Cast release  | A lunge toward screen-right with the leading arm fully extended, the curl of wind leaving the open leading hand, nothing else drawn, because the air itself is added by the game. |
...
| `melee/0.png` | Melee wind-up | Wind-up for a kick: crouched on the rear leg, arms swept back, the lead knee rising.                                                                                              |
| `melee/1.png` | Melee strike  | The kick landing: the lead leg extended straight toward screen-right, arms spread for balance, the body leaning back.                                                             |
...
Save each pick as `art/raw/unit.air.nima/<clip>/<index>.png` (indices from 0,
no gaps), then:

```bash
npm run art:normalise -- --unit unit.air.nima
npm run art:pack -- --unit unit.air.nima
...
128×192 under `art/normalised/unit.air.nima/`; look at those frames before
...
`palette` to `air`. `art:validate` checks the files the
manifest names, and CI runs it.

## Check

Run the **Sheets** section of `checklist.md` before committing. For this
sheet in particular: The shaved sides and the short orange cape. Airy, not saintly. The two air characters differ by hair: his cropped, hers in two buns.
