# Sheet: Jinu — `unit.air.jinu`

|               |                                                                                                                                                                                                                                    |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Palette**   | `air` — base `#e8dcc0`, light `#fdf6e3`, dark `#9a8e72`, accent `#fffdf5`, ink `#1b1410`, key `#00ff00`                                                                                                                            |
| **Reference** | `art/raw/reference/unit.air.jinu.png` — the full-body figure; the image reference for every frame                                                                                                                                  |
| **Deliver**   | `art/raw/unit.air.jinu/<clip>/<index>.png`, one 512×768 PNG per row of the pose table                                                                                                                                              |
| **Ships as**  | `public/art/units/jinu.png` and `jinu.json`, 128×192 frames written by `art:pack`                                                                                                                                                  |
| **Manifest**  | `'unit.air.jinu': { kind: 'sheet', atlas: 'art/units/jinu.json', pixelsPerTile: 128, footprint: { w: 1, h: 1 }, anchor: { x: 0.5, y: 0.85 }, facing: 'mirror', palette: 'air', clips: { … } }` — `art:pack` prints the whole block |
...
Talkative, delighted, about to make a point.
...
| `cast/0.png`  | Cast wind-up  | Weight drawn back onto the rear foot, the torso coiled, a single curl of wind and two small leaves at the rear hand, the sash tails lifting.                                      |
...
Save each pick as `art/raw/unit.air.jinu/<clip>/<index>.png` (indices from 0,
no gaps), then:

```bash
npm run art:normalise -- --unit unit.air.jinu
npm run art:pack -- --unit unit.air.jinu
...
128×192 under `art/normalised/unit.air.jinu/`; look at those frames before
...
sheet in particular: The two curly buns and the fringed sash. She must not be mistaken for Nima: buns, curls, more cloth in motion.
