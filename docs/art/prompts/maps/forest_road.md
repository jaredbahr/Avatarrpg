...
**Palette** hue anchors from the game's own ground (`src/render/palettes.ts`): grass `#6f9e4c`, stone `#d8cbb0`, sand `#a89880`, road `#b39064`, dirt `#b39064`, still water `#3e8fb0`. The painting replaces those tones outright, so lighter and more saturated is fine; stay in the same families so the effects and the parchment HUD still read over it.
