...
**Palette** hue anchors from the game's own ground (`src/render/palettes.ts`): stone `#565452`, wall `#3a352f`, dirt `#4d3f2f`, wood `#6b4f33`, road `#5b5044`. The painting replaces those tones outright, so lighter and more saturated is fine; stay in the same families so the effects and the parchment HUD still read over it.
