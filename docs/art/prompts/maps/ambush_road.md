...
**Palette** hue anchors from the game's own ground (`src/render/palettes.ts`): stone `#565452`, grass `#41552f`, road `#5b5044`, dirt `#4d3f2f`, still water `#3e8fb0`. The painting replaces those tones outright, so lighter and more saturated is fine; stay in the same families so the effects and the parchment HUD still read over it.
