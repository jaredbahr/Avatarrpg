...
**Palette** hue anchors from the game's own ground (`src/render/palettes.ts`): grass `#7d8850`, wood `#6b4f33`, road `#776f5d`, stone `#8a8880`, dirt `#4d3f2f`, still water `#3e8fb0`. The painting replaces those tones outright, so lighter and more saturated is fine; stay in the same families so the effects and the parchment HUD still read over it.
...
- Still water (6 tiles): a 3×1 block at columns 6–8, row 6 (middle left); a 3×1 block at columns 10–12, row 6 (the centre).
- Trees (50 tiles): 13 tiles (top right), row 0: columns 18–23; row 1: columns 22–23; rows 2–6: column 23; 13 tiles (bottom right), rows 9–13: column 23; row 14: columns 22–23; row 15: columns 18–23; 11 tiles (top left), row 0: columns 0–3; row 1: columns 0–1; rows 2–6: column 0; 11 tiles (bottom left), rows 9–13: column 0; row 14: columns 0–1; row 15: columns 0–3; one tile at column 5, row 5 (middle left); one tile at column 17, row 5 (middle right).
...
- Stone walls (14 tiles): 4 tiles (top left), row 4: columns 7–8; row 5: columns 6–7; a 4×1 block at columns 4–7, row 9 (middle left); a 2×1 block at columns 13–14, row 4 (top centre); a 2×1 block at columns 16–17, row 4 (top right); a 2×1 block at columns 14–15, row 9 (the centre).
...
- Open ground everywhere else: grass (194 tiles), bare earth (4 tiles).
