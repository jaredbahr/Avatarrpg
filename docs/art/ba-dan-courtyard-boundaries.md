...
Seven deliberately placed low props frame the plaza: four limestone planted
beds and three produce displays. Gameplay reviewed all fourteen occupied cells
against NPC approaches, both exits, spawn and the five review destinations.
...
| South market display          | (4,9), (5,9)                 | Same merchant display                           |
| Southwest garden              | (6,9), (7,9)                 | Same low planter                                |
...
while retaining exact logical canal and path registration. Permanent water is
kept out of the local ground piece and supplied by the runtime surface shader;
the transparent coping layer supplies only its stone edge, so its alpha cannot
leave a second detached water patch.
...
The lower court now carries a continuous short canal in the y6 slice: runtime
water at (6,6)–(8,6) and (10,6)–(12,6), with the existing road surface at
(9,6) as a dry footbridge crossing. The water remains the authored walkable
`~` surface; `canal-banks.webp` supplies transparent low stone coping without
introducing a wall that could strand an older save. Its 576×288 projected
envelope is derived from all six diamond centres, so the southeast run is
covered as well as the northwest run. Rows y5, y7 and y8 stay open, and route
checks cover all NPCs, both exits and the spawn.
...
The courtyard proof uses `courtyard-ground.webp`, an 1152×576 projected local
piece with transparent feathered edges. It is generated from the reviewed
material atlas by `scripts/art/ba-dan-courtyard-ground.ts`, and the coping is
generated by `scripts/art/ba-dan-canal-banks.ts`. The procedural grid remains
visible outside those pieces; the complete-map pages are not used as partial
scene authority. The superseded water-bearing canal candidate is retained only
in ignored local evidence and is not referenced.
