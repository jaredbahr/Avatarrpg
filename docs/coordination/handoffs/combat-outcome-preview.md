...
- **Worktree state:** The contact-journal correction is committed at checked
  head `00c9805`; this handoff update is the accompanying docs-only checkpoint.
- **Completed:**
  - `src/core/rules/reactions.ts` runs a bounded throwaway `BattleDraft`,
    ...
    effects, paint contacts, and status forecasts.
  - `src/core/state/battleDraft.ts` records one preview contact for each changed
    occupied cell that the real paint path processes, including lethal HP-floor
    damage and both cells of a large unit. Preview drafts do not choose chance
    status branches; normal resolver drafts retain the existing RNG behavior.
  ...
  - `src/core/rules/combatPreview.test.ts` covers deterministic prop, reaction,
    shove, boss-footprint, status, cumulative heal-cap, lethal contact,
    harmless contact, and occupied-surface scenarios.
...
- **Decisions:** Prop damage and break effects run before impact through the same
  ...
  statuses are described rather than selected. Surface contact forecasts expose
  actual HP loss after the unit's HP floor and keep one chance label per actual
  contact; repeated contacts from a large footprint are intentionally not
  collapsed into one guessed branch. Prop targets are included for push or pull
  even when blocked; ordinary surface aims do not invent a prop consequence when
  no live prop event occurs.
- **Verification:**
  - `npm run typecheck` — passed.
  - `npm run lint` — passed.
  - `npm test` — passed locally: 75 files, 718 tests on `00c9805`.
  - `npm test -- --run src/core/rules/combatPreview.test.ts src/core/rules/reactions.test.ts src/core/state/props.test.ts` — passed: 83 tests across the three focused files after the contact-journal and cumulative-cap fixes.
  ...
  - `npm run verify` — passed on `00c9805`: 75 files, 718 tests.
